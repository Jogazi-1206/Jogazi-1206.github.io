 ...
 * 포트폴리오 데이터 (Content)
 * * [수정 가이드]
 * 1. 텍스트를 수정하려면 따옴표('') 또는 백틱(``) 안의 글자를 변경하세요.
 * 2. 이미지를 추가하려면 images 배열에 경로를 넣으세요. 예: ['./images/my_img.png']
 * 3. 새로운 항목을 추가하려면 객체 블록 {}을 복사해서 붙여넣으세요.
 */
...
window.DB = {
    ...
    projects: [
        ...
        },
        {
            ...
            description: `
                <h4 class="font-bold text-navy-900 mb-2 text-lg">주제: 인바운드 관광 활성화 전략</h4>
                <p class="text-sm text-slate-600 mb-4">외국인 관광객의 편의를 위한 '초개인화 AI 슈퍼앱' 전략 제안</p>
                
                <h4 class="font-bold text-navy-900 mb-2 text-lg">Process</h4>
                <ul class="list-disc list-inside text-sm text-slate-600 space-y-2 mb-4">
                    <li><strong>시장 분석:</strong> '초개인화' 트렌드와 기존 여행 앱의 파편화 문제(Pain Point) 발견.</li>
                    <li><strong>전략 수립:</strong> AI 코스 추천, 결제 통합 등을 아우르는 '슈퍼앱' 전략 도출 및 민관 협력 모델 제시.</li>
                    <li><strong>시각화:</strong> Figma로 주요 기능과 사용자 흐름(User Flow)을 시각화하여 설득력 강화.</li>
                </ul>
            `,
